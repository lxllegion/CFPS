...
def svd_factorize(df, n_users, n_items, d):
    R = coo_matrix(
        (df.rating.values, (df.userId - 1, df.movieId - 1)),
...
def train_multivae(df, n_users, n_items, latent_dim=200, epochs=20):
    ...
    for e in range(epochs):
        print(f" MultVAE epoch {e+1}/{epochs}")
        model.train()
        for items in tqdm(user_items, desc=" users", unit="user"):
    ...
    print("Extracting MultVAE embeddings...")
    V = model.decoder[2].weight.data.cpu().numpy()  # (n_items, latent_dim)
    U = []
    model.eval()
    with torch.no_grad():
        for u in tqdm(range(1, n_users + 1), desc=" embed users", unit="user"):
...
def train_ncf(df, n_users, n_items, emb_dim=32, epochs=5):
    ...
    print("Extracting NCF embeddings...")
    U = model.user_emb.weight.data.cpu().numpy()  # (n_users, emb_dim)
    V = model.item_emb.weight.data.cpu().numpy()  # (n_items, emb_dim)
    return U, V


# -- Data loading / splitting for MovieLens 10M ----------------------------
def load_and_split(path, hold=0.2):
    # ML-10M uses "::" separator
    df = pd.read_csv(
        path,
        sep="::",
        engine="python",
...
class LinTS:
    ...
    def select(self, V):
        ...
        cov = 0.5 * (cov + cov.T)
        cov += self.init_jitter * np.eye(cov.shape[0])
...
def main():
    ...
    parser.add_argument(
        "--path", required=True, help="Path to ratings.dat (MovieLens 1M)"
    )
    parser.add_argument("--hold", type=float, default=0.2)
    parser.add_argument("--T", type=int, default=500)
    parser.add_argument("--noise", type=float, default=0.1)
    parser.add_argument(
        "--nu", type=float, default=1.0, help="Thompson sampling exploration scale"
    )
    parser.add_argument(
        "--normalize", action="store_true", help="L2-normalize U and V"
    )
    ...
    for d in dims:
        ...
        for model_name in backbones:
            ...
            for u in tqdm(range(n_u), desc=f"Users ({model_name}, d={d})"):
        ...
        for m in backbones:
            plt.plot(
                np.arange(1, args.T + 1),
                regret_curves[m],
                label=m,
            )
        plt.xlabel("Time step")
        plt.ylabel("Avg cumulative regret")
        plt.title(f"TS on CF Backbones (d={d})")
        plt.legend()
        plt.tight_layout()
        fname = f"ps_1m_compare_backbones_dim{d}.png"
        plt.savefig(fname)
        plt.close()
        print(f" → Saved {fname}")
